...
var paths = {
	...
	php: ['./*.php', './**/*.php'],
	sass: ['sass/**/*.scss', '!sass/woocommerce.scss'],
	concat_scripts: 'js/concat/*.js',
	scripts: ['js/*.js', '!js/*.min.js', '!js/responsive-menus.js'],
	sprites: 'images/sprites/*.png',
	woocommerce: 'lib/woocommerce/',
};
...
/**
 * Delete WooCommerces styles before we minify and optimize.
 */
gulp.task('clean:woocommerce', function() {
	return del(['lib/woocommerce/genesis-sample-woocommerce.css', 'lib/woocommerce/genesis-sample-woocommerce.min.css'])
});

/**
 * Compile WooCommerce Sass and run stylesheet through PostCSS.
 *
 * https://www.npmjs.com/package/gulp-sass
 * https://www.npmjs.com/package/gulp-postcss
 * https://www.npmjs.com/package/gulp-pixrem
 ...
 * https://www.npmjs.com/package/css-mqpacker
 */
gulp.task('postcss:woocommerce', ['clean:woocommerce'], function() {
	...
	.pipe(sourcemaps.init())
		...
		// Add genesis-sample prefix.
		.pipe( rename( { prefix: 'genesis-sample-' } ) )

	// Create sourcemap.
	.pipe(sourcemaps.write())

	// Create genesis-sample-woocommerce.css.
	.pipe(gulp.dest(paths.woocommerce))
	.pipe(browserSync.stream());
});
 ...
 * https://www.npmjs.com/package/gulp-cssnano
 */
gulp.task('cssnano:woocommerce', ['postcss:woocommerce'], function() {
	return gulp.src('lib/woocommerce/genesis-sample-woocommerce.css')
	.pipe(plumber({ errorHandler: handleErrors }))
	.pipe(cssnano({
		safe: true // Use safe optimizations.
	}))
	.pipe(rename('genesis-sample-woocommerce.min.css'))
	.pipe(gulp.dest(paths.woocommerce))
	.pipe(browserSync.stream());
});
...
gulp.task('watch', function() {
	...
	gulp.watch(paths.sass, ['styles', 'woocommerce']);
	gulp.watch(paths.scripts, ['scripts']);
	gulp.watch(paths.min_scripts, ['scripts']);
...
gulp.task('woocommerce', ['cssnano:woocommerce']);
gulp.task('default', ['i18n','icons', 'styles', 'scripts', 'sprites', 'woocommerce']);